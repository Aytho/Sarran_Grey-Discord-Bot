...
export default {
	...
	async execute(interaction) {
		...
		const nextHalloweenYear =
			now.getFullYear() + (now.getMonth() < 9 || (now.getMonth() === 9 && now.getDate() < 31) ? 0 : 1);
		...
		if (daysRemaining === 365) {
			...
			await interaction.reply(
				`## 🎃 Plus que ${daysRemaining} ${daysRemaining === 1 ? 'jour' : 'jours'} avant Halloween ! ${daysRemaining <= 31 ? halloweenCountdown[31 - daysRemaining] : ''}`,
			);
		}
	},
} satisfies Command;
